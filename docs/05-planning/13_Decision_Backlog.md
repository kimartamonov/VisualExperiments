...
2. Functional requirements и acceptance criteria
...
|---|---|---|---|---|
| DB-11 | Финальный состав `project.yaml` v1 | Accepted provisional | До начала Milestone 1 | Project foundation issues |
...
| DB-17 | Семантика синхронизации `frame` и `step-up` | Pending | Внутри Milestone 3 до основного implementation | Step-up implementation |
| DB-18 | Правила именования и размещения новых файлов моделей | Pending | Внутри Milestone 1 | Create/open/save model flow |
...
- `Open nuance`:
  - когда и как заполняется `defaultModel`, особенно сразу после создания проекта.
- `Impact on roadmap/issues`:
...
- `Status`: Pending
- `Question`: step-up создает верхнеуровневую модель и представительную ноду один раз или поддерживает двустороннюю/автоматическую синхронизацию?
- `Options considered`:
  - `A`: one-time generation, дальше связь только навигационная;
  - `B`: ручная регенерация по команде;
  - `C`: автоматическая синхронизация при каждом изменении frame.
- `Recommended decision for MVP`:
  - выбрать `B`: начальное создание + явная ручная регенерация или обновление позже, без live sync в первой версии.
- `Why recommended`:
  - `C` слишком рискован по сложности и по UX;
  - `A` может оказаться слишком слабым, если representation быстро устаревает;
  - `B` дает понятную границу MVP без магии.
- `Impact on roadmap/issues`:
  - до закрытия этого решения нельзя детализировать Step-up implementation issues до уровня DoD;
  - нужен отдельный `Decision` issue в начале slice step-up.
- `Due stage`: внутри Milestone 3
- `Related docs`: 10, 12

### DB-18. Правила именования и размещения новых файлов моделей

- `Status`: Pending
- `Question`: где и под каким именем создавать новые модели при create model, drill-down и step-up?
- `Options considered`:
  - `A`: всегда в `models/` root;
  - `B`: рядом с текущей моделью;
  - `C`: по предопределенным подпапкам (`research/`, `abstractions/`) по типу операции.
- `Recommended decision for MVP`:
  - `create model`: по умолчанию в текущем выбранном каталоге дерева или в `models/`;
  - `drill-down`: рядом с родительской моделью;
  - `step-up`: в выделенной верхнеуровневой папке или рядом с исходной моделью по простому правилу;
  - file name должен быть человекочитаемым slug, а identity обеспечивается `id`.
- `Why recommended`:
  - дерево проекта остается понятным;
  - путь не зависит от label как identity, но остается читабельным.
- `Impact on roadmap/issues`:
...
На текущий момент roadmap можно строить, если в начале backlog явно выделить только три decision points:

1. `DB-17` Семантика step-up sync.
2. `DB-18` Правила размещения и именования новых моделей.
3. Уточнение operational detail для `DB-11` / `DB-12` / `DB-13` в виде schema-freeze issues без изменения общей архитектуры.

Все остальные ключевые направления уже достаточно стабилизированы, чтобы переходить к capability slices и затем к issue decomposition.
